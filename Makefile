# Makefile for supply-chain-scenario-rag
...
db-start:
	podman-compose up -d supply-chain-scenario-rag-db

db-stop:
	podman-compose down supply-chain-scenario-rag-db

db-logs:
	podman-compose logs -f supply-chain-scenario-rag-db
...
build-images:
	...
	@podman build -f packages/api/Containerfile -t supply-chain-scenario-rag-backend:$(IMAGE_TAG) .
	@echo "Building frontend (UI) image..."
	@podman build -f packages/ui/Containerfile -t supply-chain-scenario-rag-frontend:$(IMAGE_TAG) .
	@echo "Building ingestion image..."
	@podman build -f packages/ingestion/Containerfile -t supply-chain-scenario-rag-ingestion:$(IMAGE_TAG) .
	@echo "✅ Images built successfully"

push-images:
	@NEED_BUILD=false && \
	if ! podman image exists supply-chain-scenario-rag-backend:$(IMAGE_TAG); then echo "Backend image not found"; NEED_BUILD=true; fi && \
	if ! podman image exists supply-chain-scenario-rag-frontend:$(IMAGE_TAG); then echo "Frontend image not found"; NEED_BUILD=true; fi && \
	...
	podman tag supply-chain-scenario-rag-backend:$(IMAGE_TAG) $$REGISTRY_URL/$(NAMESPACE)/supply-chain-scenario-rag-backend:$(IMAGE_TAG) && \
	if ! podman push $$REGISTRY_URL/$(NAMESPACE)/supply-chain-scenario-rag-backend:$(IMAGE_TAG); then echo ""; echo "❌ Failed to push backend image."; exit 1; fi && \
	echo "Tagging and pushing frontend image..." && \
	podman tag supply-chain-scenario-rag-frontend:$(IMAGE_TAG) $$REGISTRY_URL/$(NAMESPACE)/supply-chain-scenario-rag-frontend:$(IMAGE_TAG) && \
	if ! podman push $$REGISTRY_URL/$(NAMESPACE)/supply-chain-scenario-rag-frontend:$(IMAGE_TAG); then echo ""; echo "❌ Failed to push frontend image."; exit 1; fi && \
	echo "Tagging and pushing ingestion image..." && \
	podman tag supply-chain-scenario-rag-ingestion:$(IMAGE_TAG) $$REGISTRY_URL/$(NAMESPACE)/supply-chain-scenario-rag-ingestion:$(IMAGE_TAG) && \
	if ! podman push $$REGISTRY_URL/$(NAMESPACE)/supply-chain-scenario-rag-ingestion:$(IMAGE_TAG); then echo ""; echo "❌ Failed to push ingestion image."; exit 1; fi && \
	echo "✅ Images pushed successfully"

# Deployment configuration
PROJECT_NAME = supply-chain-scenario-rag
